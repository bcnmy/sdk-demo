	...
	useEffect(() => {
		let checkSessionModuleEnabled = async () => {
			...
			try {
				...
				const isEnabled1 =
					await biconomySmartAccount.isModuleEnabled(managerModuleAddr);
				setIsSessionKeyModuleEnabled(isEnabled1);
				const isEnabled2 =
					await biconomySmartAccount.isModuleEnabled(routerModuleAddr);
	...
	const createSession = async (enableModule: boolean) => {
		...
		try {
			...
			if (!isSessionKeyModuleEnabled) {
				...
				const tx1 =
					await biconomySmartAccount.getEnableModuleData(managerModuleAddr);
				transactionArray.push(tx1);
			}
			if (!isBRMenabled) {
				// -----> enableModule batched session router module
				const tx2 =
					await biconomySmartAccount.getEnableModuleData(routerModuleAddr);
